...
public class ResearchController {
    ...
    public Result<List<AiResearchTask>> getTasks(ServerWebExchange exchange) {
        Long userId = SecurityUtils.getLoginUserIdOrDefault(1L);
        return Result.success(deepResearchService.getUserTasks(userId));
    }

    @Operation(summary = "查询指定研究任务详情",
            description = "包含各阶段思考链 JSON 与最终 Markdown 调研长文")
    @GetMapping("/task/{taskId}")
    public Result<AiResearchTask> getTask(
            @Parameter(description = "任务ID（UUID）", example = "8a3f2b1c-...", required = true)
            @PathVariable String taskId) {
        AiResearchTask task = deepResearchService.getTaskByTaskId(taskId);
        if (task == null) {
            return Result.error("未找到指定的调研任务");
        }
